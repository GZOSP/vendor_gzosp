...
blacklist=('external/google' 'prebuilts/clang/host/linux-x86' 'bootable_recovery')
...
function get_repos() {
  ...
  do
    if grep -q "$i" /tmp/rebase.tmp; then # If Google has it and
      if grep -q "$i" $WORKING_DIR/manifest/$GZOSP_MANIFEST; then # If we have it in our manifest and
        if grep "$i" $WORKING_DIR/manifest/$GZOSP_MANIFEST | grep -q "remote="; then # If we track our own copy of it
