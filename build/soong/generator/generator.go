...
// Copyright (C) 2018 The gzospOS Project
//
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = gzospExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = gzospExpandVariables(ctx, dep_file)
	...
	cmd := gzospExpandVariables(ctx, String(g.properties.Cmd))
